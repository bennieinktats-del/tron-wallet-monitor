...
def historical_scan():
    ...
    This scanner intentionally uses ONE controlled request stream.  There is no
    ThreadPoolExecutor and no parallel historical API traffic.

    The scan proceeds in small chronological slices, accumulates exact A→B
    histories, evaluates recurring routines after each slice, and stops as soon
    as TARGET_PAIRS pending candidates are available.
    ...
    # One request stream only.  This is deliberately conservative because
    # TronScan was rate-limiting the previous parallel implementation.
    slice_hours = max(1, int(os.getenv("SCAN_SLICE_HOURS", "12")))
    request_delay = max(0.25, float(os.getenv("SCAN_REQUEST_DELAY", "1.0")))
    ...
    log.info("🎯 CONTROLLED TARGET-FIRST ROUTINE SCAN STARTED")
    ...
    log.info("⚡ Workers: 1 | Slice: %sh | Page size: %s | Request delay: %.2fs",
             slice_hours, page_size, request_delay)
    ...
    def evaluate_new_candidates():
        """Score accumulated exact A→B histories and announce new strong ones."""
        found_now = 0

        # Only evaluate pairs that have enough activity to possibly qualify.
        # Sorting by number of transfers makes stronger-looking relationships
        # get evaluated first, which can allow the target to be reached sooner.
        candidates = [
            (pair_key, tx_map)
    ...
    def scan_interval(interval_start, interval_end):
        """Scan one time slice sequentially, page by page."""
        ...
        while True:
            ...
            # TronScan pagination should not be driven beyond its documented
            # range.  We abandon only this unusually busy slice, then continue
            # with the next chronological slice.
            if offset >= 9500:
                log.warning(
                    "⚠️ Slice %s→%s reached the 9,500 offset safety boundary; "
            ...
            # Pace every page request.  There is intentionally no parallel work.
            time.sleep(request_delay)

    for interval_start, interval_end in intervals:
        if stop_requested or pending_count() >= target:
            break
        ...
        try:
            ...
            log.error(
                "❌ Historical slice failed (%s→%s): %s",
                interval_start, interval_end, exc
            )
            SCAN_LAST_ACTIVITY = utc_now().strftime("%Y-%m-%d %H:%M:%S UTC")
            # Keep moving through the chronological window rather than
            # immediately retrying the same failing request in parallel.
        ...
        SCAN_TWO_PLUS_PAIRS = sum(
            1 for txs in pair_stats.values() if len(txs) >= 2
        )
        ...
        log.info(
            "🔎 SLICE %s/%s | %s→%s | pages=%s processed=%s | "
            "A→B=%s | 2+=%s | new routines=%s | pending=%s/%s",
            chunk_number, len(intervals),
            interval_start.strftime("%m-%d %H:%M"),
            interval_end.strftime("%m-%d %H:%M"),
            local_pages, processed, len(pair_stats),
            SCAN_TWO_PLUS_PAIRS, new_count,
            SCAN_ROUTINE_CANDIDATES, target,
        )

        if reached_target:
            stop_requested = True
            log.info(
                "🛑 Target reached: %s/%s strong candidates. "
                "Stopping historical scan early.",
                pending_count(), target
            )
            break

        # Pace the transition between time slices too.  This makes the whole
        # scanner a single predictable request stream.
        time.sleep(request_delay)
    ...
    log.info(
        "✅ CONTROLLED TARGET-FIRST SCAN COMPLETE | processed=%s | "
        "A→B=%s | 2+=%s | pending routines=%s/%s",
        processed, len(pair_stats), SCAN_TWO_PLUS_PAIRS,
        SCAN_ROUTINE_CANDIDATES, target,
    )
