...
def human_routine_score(transfers):
    """Score whether repeated A→B transfers look like routine human activity.

    This is a heuristic, not proof of human control. A candidate is stronger
    when the SAME sender→receiver relationship repeats on multiple dates with
    roughly similar time-of-day and roughly similar amounts. CEX labels are
    deliberately NOT a disqualifier; they are only shown for manual review.
    """
    if len(transfers) < 3:
        return None
    dates = []
    ...
    for tx in transfers:
        ...
        amounts.append(transfer_amount(tx))
    distinct_dates = len(set(dates))
    # We want recurrence, but do not require three separate dates. A human can
    # have a routine relationship visible on two dates inside a 90-day window.
    if distinct_dates < 2 or len(amounts) < 3:
        return None

    # Circular time-of-day distance. Roughly the same time of day is a strong
    # human-routine signal, but we allow a fairly wide window for normal life.
    med_time = median(times)
    time_diffs = [min(abs(t - med_time), 1440 - abs(t - med_time)) for t in times]
    time_spread = median(time_diffs)
    time_score = max(0.0, 40.0 * (1.0 - min(time_spread / 240.0, 1.0)))
    ...
    # Roughly similar amounts: up to ~50% median deviation still contributes.
    amount_score = max(0.0, 40.0 * (1.0 - min(amount_spread / 50.0, 1.0)))

    span_days = (max(dates) - min(dates)).days
    recurrence_score = 15.0 if span_days >= 30 else (12.0 if span_days >= 7 else 7.0)

    # Penalize same-day bursts. Routine human transfers can repeat in a day,
    # but a large burst is more characteristic of automation.
    counts_by_date = defaultdict(int)
    for d in dates:
        counts_by_date[d] += 1
    max_day = max(counts_by_date.values())
    burst_penalty = min(10.0, max(0, max_day - 2) * 2.0)

    score = max(0.0, min(100.0, time_score + amount_score + recurrence_score - burst_penalty + 5.0))
    return {
        "score": round(score, 1),
...
def historical_scan():
    ...
    SCAN_COMPLETED_AT = "Never"
    log.info("🚀 HUMAN-ACTIVITY HISTORICAL SCAN STARTED")
    log.info("📅 Scan window: %s", scan_window_text())
    log.info("💰 Minimum transfer: $%,.2f | 🎯 Manual-verification candidates only", MIN_TRANSFER_USD)
    log.info("📡 Searching TRON TRC-20 USDT transfers for routine human-looking A→B patterns...")
    pair_stats = defaultdict(list)
    seen_global_txids = set()
    offset, page_size, processed, duplicate_pages, max_duplicate_pages, max_pages = 0, 50, 0, 0, 5, 2000
    page_number = 0
    while True:
        page_number += 1
        ...
        for tx in transfers:
            ...
            if not wallet_a or not wallet_b or wallet_a == wallet_b: continue
            ...
            # CEX tagging is informational only. A CEX-associated transfer is
            # NOT automatically rejected: if the A→B relationship itself has
            # a strong routine-human pattern, it remains a candidate for manual
            # verification. This avoids throwing away potentially useful pairs
            # just because one address has an exchange label.
            pair_stats[(wallet_a, wallet_b)].append(tx)
    ...
    for (wallet_a, wallet_b), txs in pair_stats.items():
        if len(txs) < 3: continue
        analysis = human_routine_score(txs)
        # Lower threshold intentionally: this is a CANDIDATE finder, not the
        # final human-activity judge. You will manually inspect the history.
        if not analysis or analysis["score"] < 55: continue
        tags = []
        for tx in txs:
            is_cex, tag = detect_cex(get_tag_name(tx))
            if is_cex and tag and tag not in tags:
                tags.append(tag)
        cex_name = ", ".join(tags)
    ...
    SCAN_CANDIDATE_PAIRS = len(pair_stats)
    SCAN_TWO_PLUS_PAIRS = sum(1 for txs in pair_stats.values() if len(txs) >= 2)
    # CEX labels are informational now, not a qualification gate.
    SCAN_CEX_QUALIFIED = db.execute("SELECT COUNT(*) AS count FROM human_candidates WHERE status='pending' AND cex_name != ''").fetchone()["count"]
    SCAN_LAST_ACTIVITY = SCAN_COMPLETED_AT = utc_now().strftime("%Y-%m-%d %H:%M:%S UTC")
    log.info("✅ HUMAN-ACTIVITY SCAN COMPLETE")
    log.info("📊 Processed=%s | A→B relationships=%s | 2+ transfers=%s | Human-looking candidates=%s", processed, len(pair_stats), SCAN_TWO_PLUS_PAIRS, human_candidates_found)
    log.info("👤 Pending manual verification candidates=%s | Existing approved/saved pairs=%s | Active=%s/%s", pending, saved, active, TARGET_PAIRS)
