...
def historical_scan():
    """Fast parallel historical scan for TRON TRC-20 USDT.

    Uses many independent 6-hour API windows in parallel. Busy windows are
    automatically split when they approach TronScan's 10,000-record offset
    ceiling. The goal is to finish the same 90-day scan dramatically faster
    than the old sequential scanner; actual time depends on API latency and
    rate limits and is not guaranteed to be exactly five minutes.
    """
    ...
    workers = max(4, min(int(os.getenv("SCAN_WORKERS", "24")), 48))
    page_size = 50
    base_window = timedelta(hours=6)

    log.info("🚀 FAST PARALLEL RECURRING-HUMAN-ACTIVITY SCAN STARTED")
    ...
    log.info("⚡ Parallel workers: %s | Base window: 6 hours | Page size: %s", workers, page_size)

    # Build fixed, non-overlapping base windows first. This lets the runner use
    # the network concurrently instead of waiting for one 6-hour window at a time.
    intervals = []
    cursor = start_date
    ...
    def scan_interval(interval_start, interval_end, label="6-hour base window"):
        """Scan an interval and recursively split only if it hits the API cap."""
        ...
        while True:
            ...
            try:
                ...
                # Let the future fail after the normal HTTP retry logic. The
                # main thread records it instead of sleeping 10 seconds forever.
                raise RuntimeError(
                    f"{interval_start:%Y-%m-%d %H:%M}→{interval_end:%Y-%m-%d %H:%M} "
                    f"page {local_pages}: {exc}"
                ) from exc

            transfers = data.get("token_transfers", [])
            if not transfers:
                return local_pairs, local_processed, local_pages, False, None

            for tx in transfers:
                txid = transaction_id(tx)
                # Count every unique transfer returned by the API for the
                # progress metric, while only retaining qualifying $50+ A→B
                # transfers for candidate analysis.
                local_processed += 1
            ...
            if offset >= 9500:
                ...
                if duration <= timedelta(minutes=30):
                    return local_pairs, local_processed, local_pages, True, "minimum split reached"
                midpoint = interval_start + duration / 2
                return local_pairs, local_processed, local_pages, True, (midpoint,)

            # No deliberate per-page sleep: concurrency is the speed-up.

    # Each future handles one base interval. If a busy interval hits the cap,
    # its two halves are submitted immediately to the same executor.
    ...
    executor = ThreadPoolExecutor(max_workers=workers, thread_name_prefix="scan")
    try:
        for interval_start, interval_end in intervals:
            future = executor.submit(scan_interval, interval_start, interval_end)
            pending[future] = (interval_start, interval_end)

        while pending:
            for future in as_completed(list(pending)):
                interval_start, interval_end = pending.pop(future)
                chunk_number += 1
                try:
                    local_pairs, local_processed, local_pages, hit_ceiling, split_info = future.result()
                except Exception as exc:
                    log.error("❌ Historical scan window failed: %s", exc)
                    continue

                # Merge results in the main thread only; this avoids concurrent
                # writes to the shared in-memory structures.
                ...
                label = f"{interval_start:%m-%d %H:%M}→{interval_end:%m-%d %H:%M}"
                log.info(
                    "⚡ WINDOW %s/%s complete | %s | pages=%s processed=%s | A→B=%s | 2+ transfers=%s",
                    completed_intervals, len(intervals), label, local_pages, processed,
                    len(pair_stats), SCAN_TWO_PLUS_PAIRS,
                )

                if hit_ceiling:
                    if split_info == "minimum split reached":
                        log.warning("⚠️ %s hit the 10k API ceiling at 30-minute resolution; interval may be incomplete.", label)
                    else:
                        # The parent stopped at the 9,500 offset safety boundary.
                        # Discard its partial result/count and let the two child
                        # windows provide the complete, non-overlapping data.
                        processed -= local_processed
                        total_pages -= local_pages
                        for pair_key, tx_map in local_pairs.items():
                            for tx_key in tx_map:
                                pair_stats[pair_key].pop(tx_key, None)
                            if not pair_stats[pair_key]:
                                pair_stats.pop(pair_key, None)
                        midpoint = split_info[0]
                        left = executor.submit(scan_interval, interval_start, midpoint, "automatic split")
                        right = executor.submit(scan_interval, midpoint, interval_end, "automatic split")
                        pending[left] = (interval_start, midpoint)
                        pending[right] = (midpoint, interval_end)
                break
    finally:
        executor.shutdown(wait=True)

    human_candidates_found = 0
    ...
    pending_candidates = db.execute("SELECT COUNT(*) AS count FROM human_candidates WHERE status='pending'").fetchone()["count"]
    ...
    log.info("✅ FAST PARALLEL SCAN COMPLETE")
    log.info(
        "📊 Processed=%s | A→B relationships=%s | 2+ transfers=%s | Routine candidates=%s",
        processed, len(pair_stats), SCAN_TWO_PLUS_PAIRS, human_candidates_found,
    )
    log.info(
        "👤 Pending manual verification candidates=%s | Existing saved pairs=%s | Active=%s/%s",
        pending_candidates, saved, active, TARGET_PAIRS,
    )
