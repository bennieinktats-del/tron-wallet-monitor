...
# Only a small number of strong candidates are needed for manual review.
# Keeping this low also prevents unnecessary API traffic and rate limiting.
MIN_TRANSFER_USD = float(os.getenv("MIN_TRANSFER_USD", "50"))
WEEKS_BACK = int(os.getenv("WEEKS_BACK", "2"))
HUMAN_LOOKBACK_DAYS = int(os.getenv("HUMAN_LOOKBACK_DAYS", "90"))
...
def get_pair_history_page(wallet_a, wallet_b, page=1, page_size=50, date_text=None):
    """Fetch ONE page of exact A -> B TRON TRC-20 USDT history.

    This is intentionally page-based so /candidate responds quickly instead
    of downloading up to 10,000 transfers before Telegram gets a reply.
    Returns (transactions, total_available).
    """
    page = max(1, int(page))
    ...
    if date_text:
        ...
        params["start_timestamp"] = int(day.timestamp() * 1000)
        params["end_timestamp"] = int((day + timedelta(days=1)).timestamp() * 1000) - 1
    try:
        data = http_get(
            f"{TRONSCAN_URL}/token_trc20/transfers",
            params=params, headers=tronscan_headers(), timeout=20,
        )
    except Exception as exc:
        log.warning("Pair page lookup failed for %s -> %s: %s", wallet_a, wallet_b, exc)
        return [], None
    ...
    for tx in data.get("token_transfers", []):
        if tx.get("from_address") != wallet_a or tx.get("to_address") != wallet_b:
            continue
        key = transaction_id(tx) or f"{transaction_timestamp(tx)}:{transfer_amount(tx)}"
        if key not in seen:
            seen.add(key)
            txs.append(tx)
    ...
    try:
        ...
        total = None
    return sorted(txs, key=transaction_timestamp, reverse=True), total
...
def get_pair_history(wallet_a, wallet_b, max_pages=200, page_size=50):
    ...
    Kept for scanner/legacy callers. Telegram inspection commands should use
    get_pair_history_page() so they only make one API request per page.
    """
    found = {}
    for page in range(1, max_pages + 1):
        txs, total = get_pair_history_page(wallet_a, wallet_b, page, page_size)
        if not txs:
            break
        for tx in txs:
            key = transaction_id(tx) or f"{transaction_timestamp(tx)}:{transfer_amount(tx)}"
            found[key] = tx
        if len(txs) < page_size or (total is not None and page * page_size >= total):
            break
...
def get_scan_window():
    ...
    The human-activity scanner uses a 90-day window by default. START_DATE
...
def human_routine_score(transfers):
    """Score recurring A→B activity across a 90-day window.

    Daily, weekly, biweekly and other recurring routines can qualify. Exact
    dates/times are NOT required. This is a heuristic for manual review, not
    proof that a wallet is human-controlled or that activity is criminal.
    """
    if len(transfers) < 3:
        return None
    ...
    for tx in transfers:
        ...
        times.append(dt.hour * 60 + dt.minute + dt.second / 60.0)
        amounts.append(amount)

    distinct_dates = len(set(dates))
    if distinct_dates < 2 or len(amounts) < 3:
        return None

    # Time-of-day consistency: same general period is useful, but exact time
    # is not required. Up to ~6 hours of median spread still gets credit.
    med_time = median(times)
    time_diffs = [min(abs(t - med_time), 1440 - abs(t - med_time)) for t in times]
    time_spread = median(time_diffs)
    time_score = max(0.0, 30.0 * (1.0 - min(time_spread / 360.0, 1.0)))

    # Amount consistency: reward similar recurring amounts but tolerate normal
    # human variation. Up to ~75% median deviation can still contribute.
    med_amount = median(amounts)
    if med_amount <= 0:
        return None
    amount_diffs = [abs(a - med_amount) / med_amount for a in amounts]
    amount_spread = median(amount_diffs) * 100.0
    amount_score = max(0.0, 30.0 * (1.0 - min(amount_spread / 75.0, 1.0)))

    span_days = (max(dates) - min(dates)).days
    if span_days >= 56:
        recurrence_score = 25.0
    elif span_days >= 28:
        recurrence_score = 22.0
    elif span_days >= 14:
        recurrence_score = 18.0
    elif span_days >= 7:
        recurrence_score = 14.0
    else:
        recurrence_score = 8.0

    # Repeated dates are the important part. Daily/weekly routines both get
    # credit; a large same-day burst is penalized as potentially automated.
    counts_by_date = defaultdict(int)
    for d in dates:
        counts_by_date[d] += 1
    max_day = max(counts_by_date.values())
    burst_penalty = min(12.0, max(0, max_day - 3) * 2.0)

    # Bonus for clear recurring cadence: consecutive-day activity OR repeated
    # roughly-weekly spacing. We intentionally don't demand exact periodicity.
    sorted_dates = sorted(set(dates))
    gaps = [(b - a).days for a, b in zip(sorted_dates, sorted_dates[1:])]
    cadence_bonus = 0.0
    if any(g <= 2 for g in gaps):
        cadence_bonus += 5.0
    if sum(5 <= g <= 9 for g in gaps) >= 1:
        cadence_bonus += 7.0
    if sum(10 <= g <= 21 for g in gaps) >= 1:
        cadence_bonus += 3.0

    score = max(0.0, min(100.0,
        time_score + amount_score + recurrence_score + cadence_bonus + 10.0 - burst_penalty
    ))
...
def get_human_candidates():
    return db.execute("SELECT * FROM human_candidates WHERE status='pending' ORDER BY routine_score DESC, id").fetchall()
...
def historical_scan():
    """Find only the first TARGET_PAIRS strong recurring TRON TRC-20 USDT A→B candidates.

    This is intentionally candidate-first, not a bulk relationship scanner.
    The scanner works through the historical window in small time slices,
    builds exact sender→receiver histories, scores recurring human-like
    routines, sends strong candidates to Telegram for manual evaluation, and
    stops as soon as TARGET_PAIRS pending candidates are available.

    A routine candidate is a heuristic for manual review only. It is not proof
    of human control, identity, or criminal activity.
    """
    start_date, end_date = get_scan_window()
    ...
    SCAN_STATUS = "Running — finding 5 strong routine candidates"
    ...
    page_size = 50
    # Keep concurrency deliberately modest. A 24-worker scanner was causing
    # TronScan 429s; four workers are enough to overlap network waits without
    # flooding the API.
    workers = max(2, min(int(os.getenv("SCAN_WORKERS", "4")), 8))
    base_window = timedelta(hours=6)
    target = max(1, TARGET_PAIRS)

    log.info("🎯 TARGET-FIRST ROUTINE SCAN STARTED")
    log.info("📅 Historical window: %s", scan_window_text())
    log.info("💰 Minimum transfer: $%.2f | Target strong candidates: %s", MIN_TRANSFER_USD, target)
    log.info("⚡ Workers: %s | Base window: 6 hours | Page size: %s", workers, page_size)

    # Start from the oldest date so recurrence can be built naturally across
    # the full historical window. Intervals are submitted in small batches;
    # this makes it possible to stop quickly once enough candidates are found.
    intervals = []
    cursor = start_date
    while cursor < end_date:
        nxt = min(cursor + base_window, end_date)
    ...
    def pending_count():
        return db.execute(
            "SELECT COUNT(*) AS count FROM human_candidates WHERE status='pending'"
        ).fetchone()["count"]

    def evaluate_new_candidates():
        """Score current exact A→B histories and announce newly qualified ones."""
        found_now = 0
        for (wallet_a, wallet_b), tx_map in pair_stats.items():
            pair_key = (wallet_a, wallet_b)
            if pair_key in announced:
                continue
            txs = list(tx_map.values())
            if len(txs) < 3:
                continue
            analysis = human_routine_score(txs)
            if not analysis or analysis["score"] < 55:
                continue
            ...
            for tx in txs:
                ...
                if tag:
                    tags.add(tag)
            cex_name = ", ".join(sorted(tags)) if tags else "None detected"
            save_human_candidate(wallet_a, wallet_b, txs, cex_name, analysis)
            announced.add(pair_key)
            found_now += 1

            # Send the candidate immediately for manual evaluation instead of
            # waiting for a giant historical scan to finish.
            send_telegram(
                f"🔎 <b>Routine Candidate #{pending_count()}</b>\n\n"
                f"<b>Asset:</b> TRON TRC-20 USDT\n"
                f"<b>Wallet A:</b> <code>{telegram_escape(wallet_a)}</code>\n"
                f"<b>Wallet B:</b> <code>{telegram_escape(wallet_b)}</code>\n\n"
                f"<b>Routine score:</b> {analysis['score']:.1f}/100\n"
                f"<b>Transfers:</b> {len(txs)} across {analysis['distinct_dates']} dates\n"
                f"<b>Typical amount:</b> ${analysis['amount_median']:,.2f}\n"
                f"<b>Amount variation:</b> {analysis['amount_spread_pct']:.1f}%\n"
                f"<b>Time spread:</b> {analysis['time_spread_hours']:.2f} hours\n"
                f"<b>Observed:</b> {analysis['first_seen']} → {analysis['last_seen']}\n"
                f"<b>CEX tag:</b> {telegram_escape(cex_name)}\n\n"
                f"Use <code>/candidate {pending_count()}</code> to inspect the A → B history.\n"
                f"Use <code>/approve {pending_count()}</code> only after manual evaluation."
            )

            if pending_count() >= target:
                return found_now, True
        return found_now, False

    def scan_interval(interval_start, interval_end):
        start_ms = int(interval_start.timestamp() * 1000)
        end_ms = int(interval_end.timestamp() * 1000) - 1
        offset = 0
        local_pages = 0
        local_processed = 0
        local_pairs = defaultdict(dict)
        ...
        while True:
            ...
            if not transfers:
                return local_pairs, local_processed, local_pages
            ...
            for tx in transfers:
                ...
                local_pairs[(wallet_a, wallet_b)][tx_key] = tx

            if len(transfers) < page_size:
                return local_pairs, local_processed, local_pages
            offset += page_size
            # Six-hour windows normally stay well below the TronScan 10k cap.
            # If an exceptionally busy slice reaches the cap, stop that slice
            # rather than hammering the API indefinitely.
            if offset >= 9500:
                log.warning(
                    "⚠️ %s→%s reached the 9,500 offset safety boundary; skipping remainder of this slice.",
                    interval_start.strftime("%Y-%m-%d %H:%M"), interval_end.strftime("%H:%M"),
                )
                return local_pairs, local_processed, local_pages

    # Process only a small batch at a time. This is intentionally different
    # from the old implementation that submitted the entire 90-day scan at once.
    from concurrent.futures import ThreadPoolExecutor, as_completed
    executor = ThreadPoolExecutor(max_workers=workers, thread_name_prefix="candidate-scan")
    try:
        while next_interval < len(intervals) and not stop_requested:
            batch = intervals[next_interval:next_interval + workers]
            next_interval += len(batch)
            futures = {executor.submit(scan_interval, a, b): (a, b) for a, b in batch}

            for future in as_completed(futures):
                a, b = futures[future]
                chunk_number += 1
                try:
                    local_pairs, local_processed, local_pages = future.result()
                except Exception as exc:
                    log.error("❌ Historical slice failed (%s→%s): %s", a, b, exc)
                    continue

                for pair_key, tx_map in local_pairs.items():
                    pair_stats[pair_key].update(tx_map)
                ...
                SCAN_ROUTINE_CANDIDATES = pending_count()
                SCAN_CEX_QUALIFIED = 0
                SCAN_LAST_ACTIVITY = utc_now().strftime("%Y-%m-%d %H:%M:%S UTC")

                new_count, reached_target = evaluate_new_candidates()
                SCAN_ROUTINE_CANDIDATES = pending_count()
                log.info(
                    "🔎 SLICE %s | %s→%s | pages=%s processed=%s | A→B=%s | 2+=%s | "
                    "new routines=%s | pending=%s/%s",
                    chunk_number, a.strftime("%m-%d %H:%M"), b.strftime("%m-%d %H:%M"),
                    local_pages, processed, len(pair_stats), SCAN_TWO_PLUS_PAIRS,
                    new_count, SCAN_ROUTINE_CANDIDATES, target,
                )

                if reached_target:
                    stop_requested = True
                    # Cancel work that has not started. Running API calls cannot
                    # be forcibly killed, but no new slices will be submitted.
    ...
    SCAN_STATUS = "Complete — 5 candidates sent for evaluation" if pending_count() >= target else "Complete — fewer than 5 strong candidates found"
    SCAN_CHUNK = chunk_number
    SCAN_PAGE = total_pages
    SCAN_OFFSET = 0
    SCAN_PROCESSED = processed
    ...
    log.info(
        "✅ TARGET-FIRST SCAN COMPLETE | processed=%s | A→B=%s | 2+=%s | pending routines=%s/%s",
        processed, len(pair_stats), SCAN_TWO_PLUS_PAIRS, SCAN_ROUTINE_CANDIDATES, target,
    )
...
def command_candidate(parts):
    ...
    txs, api_total = get_pair_history_page(a, b, page=page, page_size=50, date_text=date_text)
    if not txs:
        send_telegram(f"No A → B TRON TRC-20 USDT transfers found for Candidate #{n} on {date_text or 'the requested history page'}.\n\n"
                      f"Wallet A: <code>{telegram_escape(a)}</code>\nWallet B: <code>{telegram_escape(b)}</code>\n\n"
                      f"Use <code>/candidate {n}</code> for page 1 or try another page."); return
    total_pages = ((api_total + 49) // 50) if api_total else (page + (1 if len(txs) == 50 else 0))
    start_no = (page - 1) * 50 + 1
    end_no = start_no + len(txs) - 1
    subtitle=(f"Wallet A: <code>{telegram_escape(a)}</code>\nWallet B: <code>{telegram_escape(b)}</code>\n"
              f"Routine score: <b>{c['routine_score']:.1f}/100</b>\n"
              f"Candidate pattern: {c['transfer_count']} transfers across {c['distinct_dates']} dates\n"
              f"Showing A → B history: <b>{start_no}–{end_no}</b>" +
              (f" of <b>{api_total}</b>" if api_total else ""))
    if date_text: subtitle += f"\nDate: <b>{date_text} UTC</b>"
    message = f"<b>Candidate #{n} — A → B TRON TRC-20 USDT</b>\n\n{subtitle}\n\n"
    for idx, tx in enumerate(txs, start=start_no):
        ts = transaction_timestamp(tx)
        date = datetime.fromtimestamp(ts/1000, timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC") if ts else "Unknown time"
        txid = transaction_id(tx) or "Unknown"
        sender = tx.get("from_address", "Unknown")
    ...
    if total_pages and page < total_pages:
        message += f"➡️ Next page: <code>/candidate {n} {page+1}</code>"
    else:
        message += "✅ End of the available A → B history."
    send_telegram(message)
...
def startup_message():
    send_telegram(
        ...
        f"Minimum transfer: ${MIN_TRANSFER_USD:,.2f}\nHuman-pattern window: {HUMAN_LOOKBACK_DAYS} days\nRoutine threshold: 55/100\nTarget candidates: {TARGET_PAIRS}\n\n"
        "The bot will find only the first strong routine candidates and send them to Telegram for manual evaluation before monitoring."
    )
