...
def historical_scan():
    """Scan the full historical window in bounded time chunks.

    A single 90-day global TronScan query can hit the API pagination ceiling
    before reaching older transfers. We therefore scan week-by-week and merge
    the same exact A→B relationship across all chunks.
    """
    start_date, end_date = get_scan_window()
    ...
    log.info("📡 Scanning TRON TRC-20 USDT week-by-week for recurring A→B patterns...")
    ...
    while chunk_start < end_date:
        ...
        log.info("🧩 SCAN CHUNK %s | %s → %s UTC", chunk_number,
        ...
        while True:
            ...
            try:
                data = get_usdt_transfer_page(
                    start_timestamp=chunk_start_ms,
                    end_timestamp=chunk_end_ms,
                    start=offset,
                    limit=page_size,
                )
            except Exception as exc:
                log.error("Historical scan request failed in chunk %s page %s: %s", chunk_number, page_number, exc)
                time.sleep(10)
                continue
            ...
            for tx in transfers:
                ...
                if timestamp and not (chunk_start_ms <= timestamp <= chunk_end_ms):
                    continue

                # CEX tags are informational only. They never disqualify a
                # recurring A→B relationship from the human-activity radar.
            ...
            chunk_processed += new_transactions
            ...
            if page_number % 5 == 0 or len(transfers) < page_size:
                log.info(
                    "🔎 CHUNK %s | page=%s offset=%s processed=%s | A→B=%s | 2+ transfers=%s",
                    chunk_number, page_number, offset, processed,
                    len(pair_stats), SCAN_TWO_PLUS_PAIRS,
                )

            offset += page_size
            # Never allow one time chunk to run into the API's 10k offset cap.
            # If a week is this busy, split it into smaller chunks below.
            if offset >= 9500 and len(transfers) >= page_size:
                log.warning("⚠️ Chunk %s is approaching the 10k API pagination ceiling; splitting is required.", chunk_number)
                break
            if len(transfers) < page_size:
                break
            time.sleep(0.15)

        # If a week was too busy to finish safely, rescan it as one-day chunks.
        if offset >= 9500 and chunk_end - chunk_start > timedelta(days=1):
            day_start = chunk_start
            while day_start < chunk_end:
                day_end = min(day_start + timedelta(days=1), chunk_end)
                ...
                while True:
                    ...
                    for tx in txs:
                        ...
                        if amount < MIN_TRANSFER_USD:
                            continue
                        timestamp = transaction_timestamp(tx)
                        if timestamp and not (day_start_ms <= timestamp <= day_end_ms):
                            continue
                        pair_key = (wallet_a, wallet_b)
                        tx_key = txid or f"{timestamp}:{amount}:{wallet_a}:{wallet_b}"
                        pair_stats[pair_key][tx_key] = tx
                        processed += 1
                    day_offset += page_size
                    if len(txs) < page_size or day_offset >= 9500:
                        break
                    time.sleep(0.15)
                day_start = day_end
