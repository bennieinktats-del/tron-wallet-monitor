...
def historical_scan():
    ...
    def scan_interval(interval_start, interval_end, label):
        ...
        if hit_ceiling:
            ...
            if duration <= timedelta(minutes=30):
                log.warning("⚠️ Chunk %s reached the 10k ceiling even at 30-minute resolution; this interval may be incomplete.", chunk_id)
                return
    ...
    # Start with 6-hour windows to avoid repeatedly rescanning huge busy ranges.
    # Extremely busy windows are split further (down to 30 minutes) only when
    # they approach the API pagination ceiling.
    chunk_start = start_date
    base_window = timedelta(hours=6)
    while chunk_start < end_date:
        chunk_end = min(chunk_start + base_window, end_date)
        scan_interval(chunk_start, chunk_end, "6-hour base window")
