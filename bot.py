...
HUMAN_LOOKBACK_DAYS = 14  # Fixed: historical candidate search is limited to exactly two weeks
...
def historical_scan():
    ...
    def scan_interval(interval_start, interval_end, label):
        ...
        nonlocal processed, total_pages, chunk_number, stop_scan
        chunk_number += 1
...
def main():
    ...
    while True:
        ...
        pending_candidates = get_human_candidates()
        if pending_candidates:
            # Do NOT hit TronScan while candidates are awaiting manual review.
            # This keeps the API quiet and leaves Telegram responsive for
            # /candidates, /candidate and /approve. Monitoring resumes only
            # after the user approves a candidate and no pending candidates remain.
            if current_time - last_monitor >= 30:
                log.info("⏸️ Monitoring paused: %d candidate(s) awaiting manual verification.", len(pending_candidates))
