...
def human_routine_score(transfers):
    ...
    This is a heuristic, not proof of human control. The score rewards:
    - at least 3 separate calendar dates;
    - repeated transfers spread across the window;
    - roughly similar time-of-day;
    - roughly similar amounts;
    and penalizes obvious CEX tags and machine-like bursts.
    """
    if len(transfers) < 3:
        return None
    dates = []
    ...
    if distinct_dates < 3 or len(amounts) < 3:
        return None

    # Circular time-of-day distance, then median absolute distance from the
    # median time. A 90-minute spread is considered strong; 3 hours moderate.
    med_time = median(times)
    time_diffs = [min(abs(t - med_time), 1440 - abs(t - med_time)) for t in times]
    time_spread = median(time_diffs)
    time_score = max(0.0, 35.0 * (1.0 - min(time_spread / 180.0, 1.0)))
    ...
    amount_score = max(0.0, 35.0 * (1.0 - min(amount_spread / 40.0, 1.0)))

    span_days = (max(dates) - min(dates)).days
    recurrence_score = 20.0 if span_days >= 14 else (15.0 if span_days >= 7 else 8.0)
    ...
    score = max(0.0, min(100.0, time_score + amount_score + recurrence_score - burst_penalty + 10.0))
    return {
        "score": round(score, 1),
...
def historical_scan():
    ...
    while True:
        ...
        for tx in transfers:
            ...
            # Known exchange-tagged sources are kept out of the human radar.
            is_cex, _ = detect_cex(get_tag_name(tx))
            if is_cex: continue
    ...
    for (wallet_a, wallet_b), txs in pair_stats.items():
        ...
        if not analysis or analysis["score"] < 65: continue
        cex_name = ""
...
def command_candidates(parts=None):
    ...
    header = (f"<b>Human-Looking Routine Candidates</b>\n"
              ...
              f"These are suggestions only — manually verify with /history before approving.\n\n")
    blocks = []
