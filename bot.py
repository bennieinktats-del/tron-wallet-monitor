...
def command_history(parts):
    if len(parts) < 2:
        send_telegram(
            ...
            "Shows Wallet A → Wallet B TRON TRC-20 USDT history "
            "for Pair #1."
        )
        return
    ...
    # FIX:
    # Do not use only get_wallet_history(wallet_b).
    # Re-query the same global TRONSCAN TRC-20 USDT transfer source
    # used by historical_scan(), then filter it to A → B.
    matching = get_historical_pair_transactions(
        wallet_a,
        wallet_b,
        weeks_back=WEEKS_BACK,
    )
    ...
    if not matching:
        send_telegram(
            f"⚠️ <b>Pair #{display_number} — historical lookup</b>\n\n"
            f"<b>Network:</b> TRON\n"
            f"<b>Asset:</b> TRC-20 USDT\n\n"
            f"<b>Wallet A:</b>\n"
            f"<code>{telegram_escape(wallet_a)}</code>\n\n"
            f"<b>Wallet B:</b>\n"
            f"<code>{telegram_escape(wallet_b)}</code>\n\n"
            f"No matching A → B transfers were returned by the "
            f"historical TRONSCAN query for the {WEEKS_BACK}-week "
            f"window.\n\n"
            f"The pair itself remains preserved in the database."
        )
        return
    ...
    message = (
        ...
        f"<b>Volume shown:</b> ${total:,.2f}\n\n"
    )

    for index, tx in enumerate(matching, 1):
        amount = transfer_amount(tx)
        date = format_tx_date(tx)
        txid = transaction_id(tx) or "Unknown"
...
def command_final():
    """
    Show the complete current active set, using the same display
    ordering as /info and /history.
    """
    refresh_active_slots()
    active_pairs = get_active_display_pairs()
    ...
    if not active_pairs:
        send_telegram(
            "No active pairs are currently being monitored."
        )
        return

    message = (
        f"<b>Final Active Pairs — TRON TRC-20 USDT</b>\n"
...
def handle_command(text):
    ...
    if command == "info":
        ...
        send_telegram(
            ...
            "<code>history 1</code> — Show Pair #1 "
            "A → B TRON TRC-20 USDT history\n"
            "<code>final</code> — Show all current active "
            "TRON TRC-20 USDT pairs\n"
...
def poll_telegram():
    ...
    for update in updates:
        ...
        if update_id is not None:
            set_telegram_offset(int(update_id) + 1)
...
def startup_message():
    send_telegram(
        ...
        f"Target pairs: {TARGET_PAIRS}\n\n"
        f"Historical qualification and ongoing monitoring are active."
    )
...
def main():
    if not TRONSCAN_API_KEY:
        log.warning("TRONSCAN_API_KEY is not configured.")

    if not TRONGRID_API_KEY:
        log.warning("TRONGRID_API_KEY is not configured.")

    if not TELEGRAM_BOT_TOKEN:
        log.warning("TELEGRAM_BOT_TOKEN is not configured.")

    if not CHAT_ID:
        log.warning("CHAT_ID is not configured.")
