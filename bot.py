...
def historical_scan():
    """Find the first TARGET_PAIRS strong recurring TRON TRC-20 USDT A→B candidates.

    The scanner uses one controlled API request stream, but it is adaptive rather
    than artificially slow: it uses larger time slices when possible, reports
    page-by-page progress, backs off on errors/rate limits through http_get(),
    and stops immediately once the target number of pending candidates exists.

    Routine scores are heuristics for manual review only. They do not prove human
    control, identity, or criminal activity.
    """
    start_date, end_date = get_scan_window()
    ...
    # One request stream is the important protection against the previous 429s.
    # 24h slices reduce the number of time-range requests compared with the old
    # 6h/12h approach. The page delay is configurable, but never below 0.50s.
    slice_hours = max(1, int(os.getenv("SCAN_SLICE_HOURS", "24")))
    request_delay = max(0.50, float(os.getenv("SCAN_REQUEST_DELAY", "0.60")))
    progress_every_pages = max(5, int(os.getenv("SCAN_PROGRESS_PAGES", "10")))

    SCAN_STATUS = f"Running — finding {target} strong routine candidates"
    ...
    log.info("🎯 SMART CONTROLLED TARGET-FIRST ROUTINE SCAN STARTED")
    ...
    log.info("⚡ Workers: 1 | Slice: %sh | Page size: %s | Request delay: %.2fs | Progress: every %s pages",
             slice_hours, page_size, request_delay, progress_every_pages)

    intervals = []
    cursor = start_date
    step = timedelta(hours=slice_hours)
    while cursor < end_date:
        nxt = min(cursor + step, end_date)
    ...
    processed = 0
    chunk_number = 0
    last_telegram_progress = time.time()
    ...
    def evaluate_new_candidates():
        ...
        candidates = [
            (pair_key, tx_map)
            for pair_key, tx_map in pair_stats.items()
            if pair_key not in announced and len(tx_map) >= 3
        ]
        candidates.sort(key=lambda item: len(item[1]), reverse=True)

        for (wallet_a, wallet_b), tx_map in candidates:
            ...
            announced.add((wallet_a, wallet_b))
            found_now += 1
            candidate_number = pending_count()

            send_telegram(
                f"🔎 <b>Routine Candidate #{candidate_number}</b>\n\n"
                ...
                f"Use <code>/candidate {candidate_number}</code> to inspect the A → B history.\n"
                f"Use <code>/approve {candidate_number}</code> only after manual evaluation."
            )

        return found_now, pending_count() >= target

    def scan_interval(interval_start, interval_end, chunk):
        nonlocal total_pages, processed, last_telegram_progress
        global SCAN_CHUNK, SCAN_PAGE, SCAN_OFFSET, SCAN_PROCESSED
        global SCAN_CANDIDATE_PAIRS, SCAN_TWO_PLUS_PAIRS, SCAN_ROUTINE_CANDIDATES, SCAN_LAST_ACTIVITY
        ...
        log.info("🔎 START SLICE %s/%s | %s → %s UTC",
                 chunk, len(intervals),
        ...
        while True:
            ...
            if not transfers:
                log.info("📭 SLICE %s page %s returned 0 transfers — moving on.", chunk, local_pages)
            ...
            SCAN_CANDIDATE_PAIRS = len(pair_stats) + len(local_pairs)
            SCAN_TWO_PLUS_PAIRS = sum(1 for txs in pair_stats.values() if len(txs) >= 2) + sum(1 for txs in local_pairs.values() if len(txs) >= 2)
            SCAN_ROUTINE_CANDIDATES = pending_count()
            SCAN_LAST_ACTIVITY = utc_now().strftime("%Y-%m-%d %H:%M:%S UTC")

            if local_pages == 1 or local_pages % progress_every_pages == 0:
                log.info(
                    "📡 PAGE | slice=%s/%s | page=%s | offset=%s | page_transfers=%s | total_processed=%s | slice_pairs=%s | pending=%s/%s",
                    chunk, len(intervals), local_pages, offset, len(transfers), processed,
                    len(local_pairs), pending_count(), target
                )

            # Keep Telegram useful without flooding it: send at most one live
            # progress update every 45 seconds while the historical scan runs.
            now = time.time()
            if now - last_telegram_progress >= 45:
                send_telegram(
                    f"📡 <b>Scanner is moving</b>\n\n"
                    f"Asset: TRON TRC-20 USDT\n"
                    f"Slice: {chunk}/{len(intervals)}\n"
                    f"Page: {local_pages:,} | Offset: {offset:,}\n"
                    f"Transfers processed: {processed:,}\n"
                    f"A→B pairs found so far: {len(pair_stats) + len(local_pairs):,}\n"
                    f"Strong candidates: {pending_count()}/{target}\n\n"
                    f"Current slice: {interval_start.strftime('%Y-%m-%d %H:%M')} → {interval_end.strftime('%Y-%m-%d %H:%M')} UTC"
                )
                last_telegram_progress = now
            ...
            if offset >= 9500:
                log.warning(
                    "⚠️ Slice %s reached offset 9,500. Keeping collected data and moving to the next slice.", chunk
                )
                return local_pairs, local_processed, local_pages
    ...
    for interval_start, interval_end in intervals:
        ...
        chunk_number += 1
        try:
            local_pairs, local_processed, local_pages = scan_interval(
                interval_start, interval_end, chunk_number
            )
        except Exception as exc:
            log.error("❌ Historical slice failed (%s→%s): %s", interval_start, interval_end, exc)
            SCAN_LAST_ACTIVITY = utc_now().strftime("%Y-%m-%d %H:%M:%S UTC")
            send_telegram(
                f"⚠️ Scanner moved past a failed slice.\n"
                f"Slice: {interval_start.strftime('%Y-%m-%d %H:%M')} → {interval_end.strftime('%Y-%m-%d %H:%M')} UTC\n"
                f"Error: {telegram_escape(str(exc)[:300])}\n\nContinuing with the next slice."
            )
        ...
        # Evaluate immediately after each slice so a strong recurring pattern
        # is not forced to wait for the entire 90-day scan.
        ...
        log.info(
            "🔎 SLICE COMPLETE %s/%s | %s→%s | pages=%s | processed=%s | A→B=%s | 2+=%s | new=%s | pending=%s/%s",
            chunk_number, len(intervals),
            interval_start.strftime("%m-%d %H:%M"), interval_end.strftime("%m-%d %H:%M"),
            local_pages, processed, len(pair_stats), SCAN_TWO_PLUS_PAIRS,
            new_count, SCAN_ROUTINE_CANDIDATES, target
        )

        if reached_target:
            log.info("🛑 TARGET REACHED: %s/%s. Historical scanning stops now.", pending_count(), target)
            break

        time.sleep(request_delay)

    SCAN_STATUS = (
        f"Complete — {pending_count()} candidates sent for evaluation"
        if pending_count() >= target
        else f"Complete — {pending_count()} strong candidates found; target was {target}"
    )
    ...
    log.info(
        "✅ SMART CONTROLLED SCAN COMPLETE | processed=%s | A→B=%s | 2+=%s | pending=%s/%s",
