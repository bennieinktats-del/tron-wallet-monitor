...
def command_final():
    # Final numbering is ALWAYS a fresh 1..N display sequence.
    # Database IDs may start at 20, 21, 22 after earlier pairs were excluded;
    # those internal IDs must never leak into the final display.
    active_pairs = get_active_display_pairs()
    if not active_pairs:
    ...
    for display_number, pair in enumerate(active_pairs, start=1):
        message += (
            f"<b>Pair {display_number}</b>\n"
            f"Wallet A — TRON TRC-20 USDT:\n<code>{telegram_escape(pair['wallet_a'])}</code>\n"
            f"Wallet B — TRON TRC-20 USDT:\n<code>{telegram_escape(pair['wallet_b'])}</code>\n"
            f"CEX: {telegram_escape(pair['cex_name'])}\n\n"
        )
