...
def refresh_active_slots():
    """
    Keep exactly TARGET_PAIRS non-excluded pairs active whenever
    enough qualifying candidates exist.

    Excluded pairs remain permanently recorded in the candidate pool.
    When an active pair is excluded, the next available candidate is
...
def command_history(parts):
    ...
    if len(parts) < 2:
        ...
        send_telegram(
            "Usage: "
            "<code>history WALLET_ADDRESS</code>"
        )
...
def command_status():
    ...
    send_telegram(
        ...
        f"Active monitoring slots: "
        f"{min(pairs - excluded, TARGET_PAIRS)}"
        f"/{TARGET_PAIRS}\n"
